...
class AdopterProfileViewController: UIViewController {
...
//        owner.layer.backgroundColor = UIColor(red: 0.851, green: 0.851, blue: 0.851, alpha: 0.6).cgColor
        ...
        numPets.attributedText = NSMutableAttributedString(string: "~ $### in total", attributes: [NSAttributedString.Key.kern: -0.18])
        ...
        applyb.setAttributedTitle(NSMutableAttributedString(string: "Be the adopter", attributes: [NSAttributedString.Key.kern: -0.18]), for: .normal)
        ...
        if (self.pet.saved == false) {
            saveb.setImage(UIImage(systemName: "bookmark.circle.fill")?.withTintColor(.black, renderingMode: .alwaysOriginal).withConfiguration(configuration), for: .normal)} else {
                self.saved = true
