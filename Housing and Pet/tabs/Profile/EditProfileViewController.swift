...
class EditProfileViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //complete Profile
        completeProfile.text = "Complete Profile"
        ...
        profilePic.addTarget(self, action: #selector(pictureButton), for: .touchUpInside)
        ...
        //age field
        let ageFieldPlaceHolderText = "Enter age"
        ...
        //phone num field
        let phoneFieldPlaceHolderText = "Enter Phone Number"
    ...
    @objc private func nextClicked(){
        ...
        var location:String?
        var pets:Bool = false
        if yesStudentLabel.textColor == .white{
            college = true
        }
        if yesPetLabel.textColor == .white{
            pets = true
        }
        if onCampusButton.currentTitleColor == UIColor.white{
            location = "On-Campus"
        }
        else if offCampusButton.currentTitleColor == UIColor.white{
            location = "Off-Campus | Ithaca"
        }
        else if outsideIthacaButton.currentTitleColor == UIColor.white{
            location = "Outside Ithaca"
        }
        
        
        if let image = profilePic.imageView?.image {
...
//        NetworkManager.shared.createUserImage(token:ProfileViewController.backEndToken , image_data: self.base64String) { assett in
//            print("Good upload image")
//            DispatchQueue.main.async {


                NetworkManager.shared.updateUserbyToken(token: ProfileViewController.backEndToken, image_data: nil, gender: self.genderTextField.text ?? nil, age: self.ageTextField.text ?? nil, college_student:college , curr_location: location, pet_owner_boolean: pets, phone: self.phoneNumField.text ?? nil) { Appuser in
                    print("HAPPYHAPPY")
                    DispatchQueue.main.async {
...
//        }
    ...
    @objc private func pictureButton(){
